...
export default class DanteInlineTooltip extends React.Component {
  ...
  // MODIFIED: Added custom handler
  handleCustom = (clickEvent, requestBlock) => {
    const { widget_options } = requestBlock
    const render = (type, options) => {
      return this.props.onChange(addNewBlock(this.props.editorState, type, options))
    }

    return widget_options.insertion(render, this, clickEvent, requestBlock)
  }
  ...
  clickHandler =(e, type) => {
    ...
    // MODIFIED: Added custom handler
    if (typeof request_block.widget_options.insertion === 'function') {
      return this.handleCustom(e, request_block)
    }
